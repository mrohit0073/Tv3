    ...
    @Override public void onCreate(Bundle b){
        ...
        Button test=new Button(this); test.setText("TEST / CENTER CURSOR"); test.setOnClickListener(v->{
            if(RemoteAccessibilityService.isRunning()) {
                ...
                new Thread(()->{try(Socket s=new Socket("127.0.0.1",45456);BufferedWriter w=new BufferedWriter(new java.io.OutputStreamWriter(s.getOutputStream()));w.write("CURSOR_CENTER\n");w.flush();}catch(Exception ignored){}}).start();
            }
        }); root.addView(test);
